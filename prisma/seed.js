...
const decimal = (value) => new client_1.Prisma.Decimal(value.toString());
...
const buildAvailabilities = (start, totalDays, closedOffsets = []) => {
...
function main() {
    return __awaiter(this, void 0, void 0, function* () {
        const [passwordHashUser, passwordHashTenant] = yield Promise.all([
            bcryptjs_1.default.hash('user12345', 10),
            bcryptjs_1.default.hash('tenant12345', 10),
        ]);
        // Clean in dependency order to allow re-run.
        yield prisma.review.deleteMany();
        yield prisma.paymentProof.deleteMany();
        yield prisma.booking.deleteMany();
        yield prisma.roomPriceAdjustment.deleteMany();
        yield prisma.roomAvailability.deleteMany();
        yield prisma.roomImage.deleteMany();
        yield prisma.room.deleteMany();
        yield prisma.propertyImage.deleteMany();
        yield prisma.property.deleteMany();
        yield prisma.propertyCategory.deleteMany();
        yield prisma.tenantProfile.deleteMany();
        yield prisma.userToken.deleteMany();
        yield prisma.user.deleteMany();
        const categories = yield Promise.all(['Hotel', 'Apartment', 'Villa', 'Guesthouse', 'Resort'].map((name) => prisma.propertyCategory.create({ data: { name } })));
        const [tenant1, tenant2, traveler] = yield Promise.all([
            prisma.user.create({
                data: {
        ...
        const [tenantProfile1, tenantProfile2] = yield Promise.all([
        ...
        const properties = yield Promise.all([
            prisma.property.create({
                data: {
                    ...
                    description: 'Modern hotel in Jakarta CBD with coworking lounge, gym, and airport shuttle.',
            ...
            prisma.property.create({
                data: {
                    ...
                    description: 'Private villa near Canggu with pool, lounge deck, and daily housekeeping.',
        ...
        const rooms = [];
        const sunriseDeluxe = yield prisma.room.create({
            data: {
        ...
        const sunriseSuite = yield prisma.room.create({
            data: {
                propertyId: properties[0].id,
        ...
        const tropicalOneBR = yield prisma.room.create({
            data: {
        ...
        const tropicalTwoBR = yield prisma.room.create({
            data: {
                propertyId: properties[1].id,
        ...
        yield prisma.booking.create({
            data: {
                ...
                guests: 2,
                status: client_1.BookingStatus.WAITING_PAYMENT,
        ...
        yield prisma.booking.create({
            data: {
                ...
                guests: 2,
                status: client_1.BookingStatus.WAITING_CONFIRMATION,
